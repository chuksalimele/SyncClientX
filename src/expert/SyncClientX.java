...
AdjustableExitSpreadStrategy adjustableExitSpreadStrategy = AVERAGE_SPREAD;// Adjust for exit spread to max. profit
...
void computeStoploss() {
   ...
   double BuyStopLossAtStopOut = determinePriceAtStopout_New(POSITION_TYPE_BUY, open_price, symbol, total_lots, total_commission, total_swap);
   double SellStopLossAtStopOut = determinePriceAtStopout_New(POSITION_TYPE_SELL, open_price, symbol, total_lots, total_commission, total_swap);
   ...
   for(int i = 0; i < positions_total; i++) {
      ...
      if(((positionObj.PositionType() == POSITION_TYPE_SELL))) {
         ...
         //NOTE: in the case of SELL position we will add the spread to avoid premature exist thereby cheating oneself.
         //Since we know that the exit on SELL side is on the Ask price we must add the spread to be sure the
         //price hits (very close to) our expected price at stopout. This is not neccessary fo BUY side since the exit is at BID price
         StopLossAtStopOut = SellStopLossAtStopOut + ExitSpreadPoint;
         if(IsMarketJustOpen || NormalizeDouble(StopLossAtStopOut, SymbolDigits) !=  NormalizeDouble(positionObj.StopLoss(), SymbolDigits)) {
            tradeObj.PositionModify(positionObj.Ticket(), StopLossAtStopOut, positionObj.TakeProfit());
...
//|                                                                  |
//+------------------------------------------------------------------+
double determinePriceAtStopout_New(double pos, double open_price, string symbol, double total_lots, double total_commission, double total_swap) {
...
//|                                                                  |
//+------------------------------------------------------------------+
double determinePriceAtStopout(double pos, double open_price, string symbol, double total_lots, double total_commission, double total_swap) {
   double margin =  accountObj.Margin();
   double stopout_margin = margin * accountObj.MarginStopOut() / 100;
   double stopout_loss = accountObj.Balance() + accountObj.Credit() + total_commission + total_swap - stopout_margin;
   double stopout_pip_move = ammountToPips(stopout_loss, total_lots, symbol);
...
//just the case
   ...
   return amount /(lots * syb_tick_value);
}
...
//|                                                                  |
//+------------------------------------------------------------------+
void computeExitSpreadInPoint(double interval_in_hr=1.0) {
   ...
   if(adjustableExitSpreadStrategy == NO_SPREAD) {
      return;
   }


   double symbol_point = getUsableSymbolPoint(Symbol());
   double spread_in_points =  MathAbs(SymbolInfoDouble(positionObj.Symbol(), SYMBOL_ASK) - SymbolInfoDouble(positionObj.Symbol(), SYMBOL_BID)) * symbol_point;

   SpreadPointSum += spread_in_points;
   SpreadTickCount++;

   preSpreadPoint(spread_in_points);

   if(ExitSpreadPoint== 0) {
      ExitSpreadPoint = spread_in_points;
   }

   if(ExitSpreadLastTime == 0) {
      ExitSpreadLastTime = (long)TimeCurrent();//number of milliseconds that elapsed since the system start.
   }

   long elapse_in_secs = (long)TimeCurrent() - ExitSpreadLastTime;

   double elapse_in_hr = elapse_in_secs/(60 * 60.0);


   if(elapse_in_hr < interval_in_hr) {
      return;
   }

   if(PreSpreadPoint != 0) {
      ExitSpreadPoint = PreSpreadPoint;
   }

   string Msg = StringFormat("Using exit spread of %i after interval of %f hour ", (int)(ExitSpreadPoint / getUsableSymbolPoint(positionObj.Symbol())), interval_in_hr);
   Print(Msg);

   ExitSpreadLastTime = (long)TimeCurrent();//number of milliseconds that elapsed since the system start.

   preSpreadPoint(0);//initial pre spread
...
//|                                                                  |
//+------------------------------------------------------------------+
void preSpreadPoint(double spread_in_points) {

   if(spread_in_points == 0) {
      PreSpreadPoint = 0;
      return;
   }

//we are interested in the lowest spread within the give frame
   if(adjustableExitSpreadStrategy == LOWEST_SPREAD) {
      if((spread_in_points < PreSpreadPoint && spread_in_points > 0) || PreSpreadPoint == 0) {
         PreSpreadPoint = spread_in_points;
      }
   }

   if(adjustableExitSpreadStrategy == HIGHEST_SPREAD) {
      if((spread_in_points > PreSpreadPoint && spread_in_points > 0) || PreSpreadPoint == 0) {
         PreSpreadPoint = spread_in_points;
      }
   }

   if(adjustableExitSpreadStrategy == AVERAGE_SPREAD) {

      //To avoid unnecessary stoploss modification every interval let's add a tolerance of 2 pips
      // so the current average must be at least 2 pips different from the last average
      double symbol_point = getUsableSymbolPoint(Symbol());
      double prev_avg = ExitSpreadPoint;
      double current_avg = SpreadPointSum / SpreadTickCount;

      int diff = (int)(MathAbs(prev_avg - current_avg) / symbol_point);

      if(diff >= 2) {
         PreSpreadPoint = current_avg;
      }
...
void toReceivedTradePacket(string line, TradePacket &trade_packet_struct) {
   ...
   for(int i=0; i < size; i++) {
      ...
      */
      ...
      if(name == "adjustable_exit_spread_strategy") {
         if(value == "No Spread")
            adjustableExitSpreadStrategy = NO_SPREAD;
         else if(value == "Lowest Spread")
            adjustableExitSpreadStrategy = LOWEST_SPREAD;
         else if(value == "Average Spread")
            adjustableExitSpreadStrategy = AVERAGE_SPREAD;
         else if(value == "Highest Spread")
            adjustableExitSpreadStrategy = HIGHEST_SPREAD;

         Print("adjustableExitSpreadStrategy ", adjustableExitSpreadStrategy);
      }
...
//for pending orders
   for(int i=0; i < pending_orders_total; i++) {
      if(orderObj.SelectByIndex(i)) {

         cum_stoploss += orderObj.StopLoss();
         cum_target += orderObj.TakeProfit();

         if(orderObj.OrderType() == ORDER_TYPE_BUY_LIMIT) {
            buy_limit_count++;
         } else if(orderObj.OrderType() == ORDER_TYPE_BUY_STOP) {
            buy_stop_count++;
         } else if(orderObj.OrderType() == ORDER_TYPE_SELL_LIMIT) {
            sell_limit_count++;
         } else if(orderObj.OrderType() == ORDER_TYPE_SELL_STOP) {
            sell_stop_count++;
         }
   ...
   if(buy_count != BuyCount
         ||buy_limit_count != BuyLimitCount
         ||buy_stop_count != BuyStopCount
         ||sell_count != SellCount
         ||sell_limit_count != SellLimitCount
         ||sell_stop_count != SellStopCount) {
      BuyCount = buy_count;
      BuyLimitCount = buy_limit_count;
      BuyStopCount = buy_stop_count;
      SellCount = sell_count;
      SellLimitCount = sell_limit_count;
      SellStopCount = sell_stop_count;
